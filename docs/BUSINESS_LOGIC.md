...
- 系統所有時間戳均以 **UTC** 儲存（資料庫、JWT、間隔重複計時等）
- **「今日」邊界**（每日學習上限、今日完成數）使用 **UTC+8** 計算，即以 UTC+8 的午夜 00:00 作為日期分界
- 此設定由 `app/utils/constants.py` 中的 `APP_TIMEZONE` 常數控制
- 未來新增任何「今日」相關邏輯時，應統一使用 `APP_TIMEZONE` 計算日期邊界
